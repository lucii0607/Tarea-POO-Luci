...
public class Sql {
    ...
    public void sentence(String sentencia){
        try{
            ...
            System.out.println("Sql class: Sentencia realizada");
        }catch(SQLException ex){
            java.util.logging.Logger.getLogger(Principal.class.getName()).log(Level.SEVERE,null,ex);
        }
    }
    
    public ResultSet like(String tabla, String where, String like){
        ResultSet valor;
        try{
            setStatement(getConexion().createStatement());
            valor = getStatement().executeQuery("Select * from "+tabla+" where "+where+" like "+like+"");
    ...
    public void delete(String tabla, String where, String valor){
        System.out.println("DELETE FROM "+ tabla +" WHERE "+ where +" = '"+valor+"'");
        try{
